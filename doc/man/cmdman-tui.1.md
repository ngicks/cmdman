...
- `switcher`: every known compose project — running, exited, and never run —
  ...
  removed M` — is reported there when it ends. The widget also stands where the
  active project stands: each time that mark resolves it moves itself into the
  project's directory, so the pane's own path — and any multiplexer binding
  keyed on it — follows the project the window is showing rather than wherever
  the widget was started from.
