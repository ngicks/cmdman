...
func (e *Emulator) SendMouse(m Mouse) {
	...
	// uv.MouseButton aliases the upstream x/ansi MouseButton; the vendored
	// EncodeMouseButton wants this fork's. Both are byte, so convert.
	b := ansi.EncodeMouseButton(ansi.MouseButton(mouse.Button), isMotion,
