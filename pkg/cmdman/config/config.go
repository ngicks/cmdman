// Package config holds cmdman's process-wide configuration
// (CmdmanConfig), its path-helper methods, the CMDMAN_* environment
// variable names, and the config-file/XDG resolution logic. It is a leaf
// package so both pkg/cmdman and pkg/cmdman/monitor can depend on it
// without a cycle.
package config
